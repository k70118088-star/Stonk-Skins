import React from "react";
import { FaWallet } from "react-icons/fa";
import { FaCaretDown } from "react-icons/fa";
import { FaUserAlt } from "react-icons/fa";
import Icons from "./Icons";
import Category from "./Category";

const Navbar = () => {
  return (
    <div className="w-full mx-auto bg-white/30 backdrop-blur-[10px]">
      <div className="max-w-287 mx-auto w-full flex flex-col items-center justify-center pt-3">
        <div className="flex items-center justify-between w-full mb-4">
          <div className="flex items-center gap-7.75">
            <img src="/assets/loho.webp" alt="" className="w-29 h-13.75" />
            <div className="flex items-center backgroung-img relative max-w-[320px] w-full max-h-13">
              <input
                type="search"
                placeholder="Search Skins..."
                className="py-4 w-[268px] h-[52px]  pl-8 font-medium text-base leading-[125%] text-[#F5F5F56E]/43 bg-[#FFFFFF0F]/6 rounded-[68px] "
              />
              <button className="flex items-center justify-center left-66.5 bg-[#FFFFFF0F]/6  h-13 w-13 rounded-[500px]">
                <Icons icon={"search"} />
              </button>
            </div>
          </div>
          <div className="flex items-center justify-center">
            <p className="font-medium text-base text-[#F5F5F5] leading-[125%] mr-16">
              Market
            </p>
            <p className="font-bold text-base leading-[150%] text-[#F5F5F5] flex items-center gap-2 mr-8">
              <span className="text-[#FFFFFF]/10">
                <FaWallet />
              </span>
              $0
            </p>
            <div className="flex items-center">
              <p className="font-normal text-base leading-[150%] text-[#F5F5F5] mr-2">
                John Doe
              </p>
              <div className="h-11 w-11 img flex items-center justify-center text-[#FFFFFF]">
                <FaUserAlt />
              </div>
              <p className="text-white"><FaCaretDown /></p>
            </div>
          </div>
        </div>
        <div className="border-t border-t-[#43344C8C] w-full pt-3.5 pb-4.5">
          <Category />
        </div>
      </div>
    </div>
  );
};

export default Navbar;
